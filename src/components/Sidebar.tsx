...
import { Linkedin, Github, ExternalLink, Menu } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ onSelect }) => {
  ...
  return (
    <div className="bg-[#254879] text-white w-full md:w-1/6 md:h-full">
      <div className="md:hidden flex justify-between items-center p-4">
        <h2 className="text-xl font-bold">James B Hill</h2>
        <button onClick={toggleMenu} className="text-white">
          <Menu size={24} />
        </button>
      </div>
      <div className={`${isMenuOpen ? 'block' : 'hidden'} md:block`}>
        <ul className="flex flex-col md:flex-col space-y-0 md:space-y-2 p-4">
